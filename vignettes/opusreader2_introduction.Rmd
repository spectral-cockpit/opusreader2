...
acquired through the OPUS spectroscopy software. These files have numerical
extensions, with incrementing extension numbers based on the naming scheme
...
an example file produced by a Bruker ALPHA® spectrometer that operates in
...
but are present in the file (more about it below).
...
viewer. To learn more about the respective block names and what data and
parameters are associated with it, we recommend reading the help via
`?read_opus`. Here we show basic examples.
...
tab. Here we use base R subsetting, `names()` and `str()` to examine the
...
read all OPUS files present in below a certain folder level. We illustrate it
with all OPUS test files that come with {opusreader2}, which are are also used
in unit tests.
